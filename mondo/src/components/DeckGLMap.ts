...
export class DeckGLMap {
  ...
  private allFlightsData: Array<{ id: string; callsign: string; lat: number; lon: number; altitude: number; velocity: number; heading: number; verticalRate: number; squawk: string; originCountry: string; airline: string }> = [];
  ...
  private buildLayers(): LayersList {
    ...
    // Military vessels layer (shown by 'military' combined OR 'militaryVessels' standalone)
    if ((mapLayers.military || mapLayers.militaryVessels) && filteredMilitaryVessels.length > 0) {
      layers.push(this.createMilitaryVesselsLayer(filteredMilitaryVessels));
    }

    // Military vessel clusters layer
    if ((mapLayers.military || mapLayers.militaryVessels) && filteredMilitaryVesselClusters.length > 0) {
      layers.push(this.createMilitaryVesselClustersLayer(filteredMilitaryVesselClusters));
    }

    // Military flights layer (shown by 'military' combined OR 'militaryFlights' standalone)
    if ((mapLayers.military || mapLayers.militaryFlights) && filteredMilitaryFlights.length > 0) {
      layers.push(this.createMilitaryFlightsLayer(filteredMilitaryFlights));
    }

    // Military flight clusters layer
    if ((mapLayers.military || mapLayers.militaryFlights) && filteredMilitaryFlightClusters.length > 0) {
  ...
  private createAllFlightsLayer(): IconLayer {
    ...
    const getFlightColor = (alt: number): [number, number, number, number] => {
      ...
      if (alt < 2000) return [255, 200, 50, 220];
      if (alt < 5000) return [255, 160, 0, 220];
      if (alt < 8000) return [0, 200, 80, 220];
      if (alt < 10000) return [0, 160, 255, 220];
      return [160, 80, 255, 220];
    };
    ...
    return new IconLayer<AFD>({
      ...
      getIcon: () => ({ url: planeIconUrl, width: 36, height: 36, anchorY: 18, anchorX: 18, mask: true }),
      getSize: 22,
      sizeMinPixels: 10,
      sizeMaxPixels: 28,
  ...
  public setAllFlights(flights: Array<{ id: string; callsign: string; lat: number; lon: number; altitude: number; velocity: number; heading: number; verticalRate: number; squawk: string; originCountry: string; airline: string }>): void {
    this.allFlightsData = flights;
    this.render();
  }
