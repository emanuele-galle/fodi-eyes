...
export class App {
  ...
  private createPanels(): void {
    ...
    // Market Intelligence Panels (crypto panels removed)
  ...
  private async loadAllData(): Promise<void> {
    ...
    if (this.mapLayers.liveFlights) tasks.push({ name: 'liveFlights', task: runGuarded('liveFlights', () => this.loadAllFlights()) });
  ...
  private async loadNews(): Promise<void> {
    ...
    // Download banner disabled — Fodi Eyes is web-only
    // maybeShowDownloadBanner();
    // mountCommunityWidget(); // Disabled — external WorldMonitor discussion link
  ...
  private allFlightsRefreshId: ReturnType<typeof setInterval> | null = null;
  ...
  private async loadAllFlights(): Promise<void> {
    try {
      const resp = await fetch('/api/flights/global', {
        ...
        body: JSON.stringify({
          boundingBox: {
            southWest: { latitude: -60, longitude: -180 },
            northEast: { latitude: 70, longitude: 180 },
          },
        }),
      ...
      const items = (data.items || []) as Array<{ icao24: string; callsign: string; lat: number; lon: number; altitude: number; velocity: number; heading: number; verticalRate: number; squawk: string | null; originCountry: string }>;

      const AIRLINE_CODES: Record<string, string> = {
        RYR: 'Ryanair', EZY: 'easyJet', WZZ: 'Wizz Air', VLG: 'Vueling',
        AZA: 'ITA Airways', THY: 'Turkish Airlines', AFR: 'Air France',
        BAW: 'British Airways', DLH: 'Lufthansa', SWR: 'Swiss',
        TAP: 'TAP Portugal', IBE: 'Iberia', KLM: 'KLM',
        UAE: 'Emirates', QTR: 'Qatar Airways', UPS: 'UPS', FDX: 'FedEx',
        AAL: 'American Airlines', DAL: 'Delta', UAL: 'United',
        SIA: 'Singapore Airlines', CPA: 'Cathay Pacific',
        RCH: 'USAF', IAM: 'Italian AF', GAF: 'German AF',
      };
      const getAirline = (cs: string) => AIRLINE_CODES[cs.substring(0, 3).toUpperCase()] || '';

      const flights = items.map(f => ({
        id: f.icao24,
      ...
      this.map?.setAllFlights(flights);
      this.map?.setLayerReady('liveFlights', flights.length > 0);
      console.log(`[App] Loaded ${flights.length} live flights`);
    } catch (e) {
      console.error('[App] Live flights load failed:', e);
      this.map?.setLayerReady('liveFlights', false);
    }

    // Auto-refresh every 2 minutes
    if (!this.allFlightsRefreshId && this.mapLayers.liveFlights) {
      this.allFlightsRefreshId = setInterval(() => {
        if (this.mapLayers.liveFlights && !this.isDestroyed) {
          void this.loadAllFlights();
        } else if (this.allFlightsRefreshId) {
          clearInterval(this.allFlightsRefreshId);
          this.allFlightsRefreshId = null;
        }
      }, 2 * 60 * 1000);
    }
  }
