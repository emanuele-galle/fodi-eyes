...
async function initSettingsWindow(): Promise<void> {
  ...
  const wmMount = document.getElementById('fodieyesApp');
  if (!llmMount || !apiMount) return;

  // Mount API key tab immediately — it doesn't depend on secrets
  ...
  // Load secrets then refresh Fodi Eyes tab to reflect actual key status
