...
export function generateStoryDeepLink(
  ...
  return `https://fodi-eyes.fodivps2.cloud/api/story?${params.toString()}`;
}
...
export const shareTexts = {
  ...
  linkedin: (data: StoryData) =>
    ...
    `\nData via Fodi-eyes - Intelligence Dashboard Italia`,
...
export function getShareUrls(data: StoryData): Record<string, string> {
  ...
  return {
    ...
    reddit: `https://reddit.com/submit?url=${encodeURIComponent(url)}&title=${encodeURIComponent(`${data.countryName} Intelligence Brief - Fodi-eyes`)}`,
