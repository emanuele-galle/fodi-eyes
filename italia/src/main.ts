...
// import { inject } from '@vercel/analytics'; // Disabled — not on Vercel
import { App } from './App';
...
Sentry.init({
  ...
  release: `fodi-eyes@${__APP_VERSION__}`,
  environment: location.hostname === 'fodi-eyes.fodivps2.cloud' ? 'production'
...
// Vercel Analytics disabled — not on Vercel
// inject();
...
// Debug helpers for geo-convergence testing — only in development
...
Object.defineProperty(window, 'beta', {
  get() {
    const on = localStorage.getItem('fodi-eyes-beta-mode') === 'true';
    console.log(`[Beta] ${on ? 'ON' : 'OFF'}`);
    return on;
  },
  set(v: boolean) {
    if (v) localStorage.setItem('fodi-eyes-beta-mode', 'true');
    else localStorage.removeItem('fodi-eyes-beta-mode');
    location.reload();
  },
});
