...
interface LiveChannel {
  ...
  externalUrl?: string; // Direct link to external streaming page (fallback when embed fails)
  hlsChannel?: string; // Server-side HLS channel ID (for Italian TV via /api/tv-player)
}

// Live channels: Italian first, then international (24/7 live streams)
const FULL_LIVE_CHANNELS: LiveChannel[] = [
  // Italian channels
  { id: 'rainews', name: 'Rai News 24', handle: '', fallbackVideoId: '', hlsChannel: 'rainews' },
  { id: 'skytg24', name: 'Sky TG24', handle: '@SkyTG24', fallbackVideoId: '' },
  { id: 'euronews-it', name: 'Euronews IT', handle: '@euronewsit', fallbackVideoId: '' },
...
// Italian-only channel IDs for the Italia view
const ITALIAN_CHANNEL_IDS = new Set(['rainews', 'skytg24', 'euronews-it']);

const LIVE_CHANNELS = FULL_LIVE_CHANNELS;
const LIVE_CHANNELS_ITALIA = FULL_LIVE_CHANNELS.filter(ch => ITALIAN_CHANNEL_IDS.has(ch.id));
...
export class LiveNewsPanel extends Panel {
  ...
  constructor(options?: { italiaOnly?: boolean }) {
    super({ id: 'live-news', title: t('panels.liveNews') });
    this.channels = options?.italiaOnly ? LIVE_CHANNELS_ITALIA : LIVE_CHANNELS;
  ...
  private get embedOrigin(): string {
    try { return new URL(getRemoteApiBaseUrl()).origin; } catch { return 'https://fodi-eyes.fodivps2.cloud'; }
  }
  ...
  private static resolveYouTubeOrigin(): string | null {
    const fallbackOrigin = 'https://fodi-eyes.fodivps2.cloud';
  ...
  private async switchChannel(channel: LiveChannel): Promise<void> {
    ...
    // HLS channels (Italian TV) skip YouTube entirely
    if (channel.hlsChannel) {
      this.destroyPlayer();
      this.embedHlsStream(channel);
  ...
  private showOfflineMessage(channel: LiveChannel): void {
    // HLS channels use direct stream player
    if (channel.hlsChannel) {
      this.embedHlsStream(channel);
      return;
    }
  ...
  private embedHlsStream(channel: LiveChannel): void {
    if (!channel.hlsChannel) return;
    this.content.innerHTML = '';
    ...
    iframe.src = `/api/tv-player?channel=${encodeURIComponent(channel.hlsChannel)}`;
    iframe.title = `${channel.name} - Diretta`;
  ...
  private async initializePlayer(): Promise<void> {
    // HLS channels (Italian TV) use server-side player
    if (this.activeChannel.hlsChannel) {
      this.embedHlsStream(this.activeChannel);
      return;
    }
