...
export class Sidebar {
  ...
  private activeTabs: TabConfig[];
  private activeTab: string;
  private collapsed = false;
  private tabPanels: Map<string, Panel[]> = new Map();

  constructor(customTabs?: TabConfig[]) {
    this.activeTabs = customTabs || TABS;
    this.activeTab = this.activeTabs[0]?.id ?? 'news';
  ...
  private renderTabs(): void {
    ...
    for (const tab of this.activeTabs) {
      ...
      btn.title = tab.tooltip || tab.label;
  ...
  registerPanels(tab: string, panels: Panel[]): void {
