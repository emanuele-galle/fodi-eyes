...
import { fetchFlightsForMap } from '@/services/flights';
...
import { PanelToggleSettings } from '@/components/PanelToggleSettings';
...
export class App {
  ...
  public destroy(): void {
    ...
    if (this.flightsRefreshId) {
      clearInterval(this.flightsRefreshId);
      this.flightsRefreshId = null;
    }
  ...
  private createPanels(): void {
    ...
    // === SIDEBAR (default for initial view, will be swapped by router) ===
    ...
    // Mount sidebar into the main-content (before views take over)
    const mainContent = this.container.querySelector('.main-content');
    if (mainContent) {
      mainContent.appendChild(this.sidebar.getElement());
    }
    ...
    container.querySelectorAll('.panel-toggle-item').forEach((item) => {
      item.addEventListener('click', () => {
        ...
        if (config) {
          ...
          this.renderPanelToggles();
          this.applyPanelSettings();
          // Sync in-sidebar toggle panel
          (this.panels['panel-toggle-settings'] as PanelToggleSettings | undefined)?.renderToggles();
  ...
  private async loadAllData(): Promise<void> {
    ...
    if (this.mapLayers.flights) tasks.push({ name: 'flights', task: runGuarded('flights', () => this.loadFlights()) });
  ...
  private flightsRefreshId: ReturnType<typeof setInterval> | null = null;

  private async loadFlights(): Promise<void> {
    try {
      const flights = await fetchFlightsForMap();
      this.map?.setFlights(flights);
      this.map?.setLayerReady('flights', flights.length > 0);
      this.statusPanel?.updateApi('Flights', { status: flights.length > 0 ? 'ok' : 'error' });
      console.log(`[App] Loaded ${flights.length} flights`);
    } catch (e) {
      console.error('[App] Flights load failed:', e);
      this.map?.setLayerReady('flights', false);
      this.statusPanel?.updateApi('Flights', { status: 'error' });
    }

    // Auto-refresh every 2 minutes while the layer is active
    if (!this.flightsRefreshId && this.mapLayers.flights) {
      this.flightsRefreshId = setInterval(() => {
        if (this.mapLayers.flights && !this.isDestroyed) {
          void this.loadFlights();
        } else if (this.flightsRefreshId) {
          clearInterval(this.flightsRefreshId);
          this.flightsRefreshId = null;
        }
      }, 2 * 60 * 1000);
